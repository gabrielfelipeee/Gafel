...
namespace UseCases.Test.BankAccount.GetById;

public class GetBankAccountByIdUseCaseTest
{
    [Fact]
    public async Task Success()
    {
        ...
        var useCase = CreateUseCase(user: user, bankAccount: bankAccount, person: person);
    ...
    private static GetBankAccountByIdUseCase CreateUseCase(
        UserDto user,
        Gafel.Domain.Entities.Person? person = null,
        Gafel.Domain.Entities.BankAccount? bankAccount = null)
    {
        ...
        if (person is not null && bankAccount is not null)
            bankAccountReadOnlyRepository.GetById(person, bankAccount);
